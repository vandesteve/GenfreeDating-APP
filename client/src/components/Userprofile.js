...
import '../css/Lightbox.css'; // Import the lightbox CSS

const themelike = createTheme({
    palette: {
        primary: {
            main: '#4CBB17', // Green for like button
        },
        secondary: {
            main: '#F5F5F5',
        },
...
const themeunlike = createTheme({
    palette: {
        primary: {
            main: '#FF4081', // Updated to match lightbox pink
        },
        secondary: {
            main: '#F5F5F5',
        },
...
const UserProfile = () => {
    ...
    const ProfileData = {
        ...
        'Tags:': userData.tags ? userData.tags.map((tag, i) => ((i ? ', ' : '') + tag)) : '',
    };
    ...
    if (userLists.connected.includes(Number(params.id))) {
        likeButton = (
            <>
                <Button theme={themeunlike} onClick={() => unlikeUser(params.id)} variant="contained" sx={{ mr: 1 }}>
                    Unlike user
                </Button>
                <Button onClick={() => navigate('/chat')} variant="contained" sx={{ backgroundColor: '#FF4081', '&:hover': { backgroundColor: '#F50057' } }}>
                    Connected
                </Button>
            </>
        );
        ...
        likeButton = (
            <Button theme={themeunlike} onClick={() => unlikeUser(params.id)} variant="contained">
                Unlike user
            </Button>
        );
    } else {
        likeButton = (
            <Button theme={themelike} onClick={() => likeUser(params.id)} variant="contained">
                Like user
            </Button>
        );
    }
        ...
        <Container maxWidth='md' sx={{ pt: 5, pb: 5 }}>
            <Paper elevation={10} sx={{ padding: 3 }}>
                <Grid sx={{
                    ...
                    <UserAvatar
                        userData={userData}
                        onClick={() => openLightbox(userData.profile_pic?.picture_data)} // Open lightbox
                        sx={{ cursor: 'pointer' }} // Ensure clickable
                    />
                    <Box sx={{ width: 'fit-content', ml: 5 }}>
                        ...
                        <Box sx={{ mt: 2 }}>
                            ...
                            <Button
                                ...
                                variant="contained"
                                sx={{ mr: 1, backgroundColor: '#FF4081', '&:hover': { backgroundColor: '#F50057' } }}
                            >
                                Block user
                            </Button>
                            <Button
                                theme={themeunlike}
                                onClick={() => reportUser(params.id)}
                                variant="contained"
                ...
                <Grid container spacing={2} sx={{ mb: 3 }}>
                    {other_pictures.map((picture, i) => (
                        <Grid item xs={6} sm={4} md={3} key={picture.picture_id}>
                            <img
                                ...
                                onClick={() => openLightbox(picture.picture_data)} // Open lightbox
